...
public class PerfumeServicesImpl implements PerfumeServices {
    ...
    public List<PerfumeDto> getAllPerfumes() {
        // new ProductRepoImpl().getAllProducts(null);
        return perfumes;
    }

    public List<PerfumeDto> getAllPerfumes(EntityManager em) {
        System.out.println("From PerfumeImpl: ");
        List<PerfumeDto> perf = new ArrayList<>();
        for(Product p : new ProductRepoImpl().getAllProducts(em)) {
            System.out.println(p.getName()+" " + p.getProductId());
            perf.add(ProductMapper.toDto(p));
        }
        return perf ;
    }
